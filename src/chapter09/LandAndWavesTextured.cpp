#include "LandAndWavesTextured.h"
...
LandAndWavesTextured::~LandAndWavesTextured()
{
...
void LandAndWavesTextured::onMouseDown(int16_t xPos, int16_t yPos, uint8_t /*buttons*/)
{
...
void LandAndWavesTextured::onMouseUp(int16_t xPos, int16_t yPos, uint8_t /*buttons*/)
{
    m_curMouseX = m_lastMouseX = xPos;
    m_curMouseY = m_lastMouseY = yPos;
    ReleaseCapture();
...
void LandAndWavesTextured::onMouseMove(int16_t xPos, int16_t yPos, uint8_t buttons)
{
    m_curMouseX = xPos;
    m_curMouseY = yPos;
...
void LandAndWavesTextured::initialize()
{
...

    {
        ...
        Microsoft::WRL::ComPtr<ID3DBlob> pVertexShader = D3D12Util::compileShader(L"data/shaders/chapter09/landAndWavesTextured.hlsl", "vs", "vs_5_1");
        desc.VS.pShaderBytecode = pVertexShader->GetBufferPointer();
        desc.VS.BytecodeLength = pVertexShader->GetBufferSize();

        Microsoft::WRL::ComPtr<ID3DBlob> pPixelShader = D3D12Util::compileShader(L"data/shaders/chapter09/landAndWavesTextured.hlsl", "ps", "ps_5_1");
...
void LandAndWavesTextured::update(float dt)
{
    m_camera.update();
...
void LandAndWavesTextured::render()
{
